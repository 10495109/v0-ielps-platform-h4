...
import { useEilps, type DataSource } from '@/lib/use-eilps'
import type {
  Panel as PanelType,
  StatItem,
  ListItem,
...
export function Panel({
  ...
  const { data, source, error } = useEilps<unknown>(
    ...
    panel.sample,
    (raw) => {
      const out = panel.transform ? panel.transform(raw) : normalizePanelData(panel.kind, raw)
      if (!isPanelShape(panel.kind, out)) throw new Error('panel_contract_mismatch')
      return out
    },
  )
...
function unwrapRecord(value: unknown): Record<string, unknown> {
  ...
  for (const key of ['data', 'result', 'summary', 'state', 'dashboard', 'subscription']) {
...
function findArray(value: unknown): unknown[] {
  ...
  for (const key of ['items', 'results', 'rows', 'classes', 'bookings', 'projects', 'tutors', 'rewards', 'assignments', 'payouts', 'content', 'catalogue', 'lessons']) {
...
/** Generic normalization never invents a value; every rendered field is copied from the response. */
function normalizePanelData(kind: PanelType['kind'], raw: unknown): unknown {
  if (kind === 'note') return raw
  if (kind === 'stat') {
    return Object.entries(unwrapRecord(raw))
      .filter(([, value]) => value == null || ['string', 'number', 'boolean'].includes(typeof value))
      .slice(0, 8)
      .map(([key, value]) => ({ label: humanize(key), value: scalar(value) || 'Not reported' }))
  }
  const rows = findArray(raw).filter((item) => item && typeof item === 'object') as Record<string, unknown>[]
  if (kind === 'table') {
    const columns = [...new Set(rows.flatMap((row) => Object.keys(row)))].slice(0, 5)
    return { columns: columns.map(humanize), rows: rows.slice(0, 20).map((row) => columns.map((column) => scalar(row[column]))) }
  }
  if (kind === 'timeline') {
    return rows.slice(0, 12).map((row, index) => ({
      time: scalar(row.time || row.startsAt || row.start || row.createdAt) || String(index + 1),
      title: scalar(row.title || row.name || row.status || row.id),
      detail: scalar(row.detail || row.description || row.level || row.type),
    }))
  }
  if (kind === 'cards') {
    return rows.slice(0, 12).map((row) => ({
      title: scalar(row.title || row.name || row.label || row.id),
      subtitle: scalar(row.subtitle || row.level || row.type || row.status),
      body: scalar(row.summary || row.description || row.body || row.topic),
      tag: scalar(row.tag || row.status),
      image: scalar(row.image || row.imageUrl) || undefined,
    }))
  }
  return rows.slice(0, 20).map((row) => ({
    title: scalar(row.title || row.name || row.label || row.status || row.id),
    subtitle: scalar(row.subtitle || row.description || row.level || row.type),
    meta: scalar(row.meta || row.updatedAt || row.createdAt || row.count),
...
function OperationalState({
  ...
  const copy: Partial<Record<DataSource, string>> = {
    ...
    admin: 'An administrator role is required to view these records.',
    provider: 'The required external provider has not been configured.',
    not_implemented: 'A canonical server route has not been wired for this panel.',
    unavailable: 'The IELPS server could not provide this panel.',
  }
  return (
    <div className="rounded-xl border border-border bg-soft p-4 text-sm text-muted-foreground">
      <p>{copy[source] || 'This data is unavailable.'}</p>
