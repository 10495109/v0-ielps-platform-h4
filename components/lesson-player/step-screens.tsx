...
function AudioChip({ text, accent }: { text: string; accent: AccentToken }) {
  const a = ACCENT[accent]
...
export function StepScreen(props: StepProps) {
  ...
  switch (step.kind) {
    ...
    case 'check':
      return <QuizStep {...props} items={props.lesson.checkItems} />
    case 'recap':
...
function IntroStep({ step, lesson, accent, junior, onDone, juniorReadability }: StepProps & { junior?: boolean }) {
  const a = ACCENT[accent]
...
function MemoryReviewStep({ step, lesson, accent, juniorReadability, onDone }: StepProps) {
  ...
  function grade(correct: boolean) {
    void import('@/lib/adapters').then(({ reviewApi }) =>
      reviewApi.submitReview({ word: card?.word, correct }),
    )
    ...
    <StepShell step={step} accent={accent} junior={juniorReadability}>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => grade(false)} className="rounded-full border border-border px-4 py-2 text-sm font-medium hover:bg-soft">
            Hard
          </button>
          <button type="button" onClick={() => grade(true)} className={`rounded-full px-4 py-2 text-sm font-semibold ${a.solid}`}>
            I remember
          </button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        ...
        {scored ? (
          <div className="mt-4 rounded-xl bg-soft p-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-foreground">Pronunciation score</span>
              <span className={`text-lg font-bold ${a.text}`}>86%</span>
            </div>
            <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-border">
              <div className={`h-full ${a.dot}`} style={{ width: '86%' }} />
            </div>
            {aiHelpEnabled ? (
              <p className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground">
                <Sparkles className={`size-3.5 ${a.text}`} /> AI coach: stress the first syllable in “commute”.
              </p>
            ) : null}
          </div>
        ) : null}
...
function WritingStep({ step, lesson, accent, juniorReadability, onDone }: StepProps) {
  ...
  function submit() {
    void import('@/lib/adapters').then(({ writingApi }) =>
      writingApi.submitAttempt({ prompt, response: text }),
    )
...
function ErrorFixStep({ step, lesson, accent, juniorReadability, onDone }: StepProps) {
  ...
  function next() {
    void import('@/lib/adapters').then(({ feedbackApi }) =>
      feedbackApi.getErrorSupport({ sentence: item?.wrong }),
    )
