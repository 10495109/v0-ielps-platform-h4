...
export const junior: AccountApp = {
      ...
      panels: [
        {
          ...
          transform: (raw) => {
            const o = (raw ?? {}) as Record<string, unknown>
            return [
              { label: 'Lessons done', value: String(o.lessonsCompleted ?? 'Not reported') },
              { label: 'Day streak', value: String(o.streak ?? 'Not reported') },
              { label: 'Stars', value: String(o.stars ?? 'Not reported') },
            ]
          },
          sample: [
        ...
        },
        {
          ...
          transform: (raw) => {
            ...
            return [{ title: 'My English course', subtitle: `${summary.levels} learning levels`, body: `${summary.lessons} lessons are ready`, tag: 'Play' }]
          },
          sample: [
      ...
      panels: [
        {
          ...
          kind: 'cards',
          endpoint: { method: 'GET', path: '/api/practice/catalog?level=A1&limit=6' },
          transform: (raw) =>
  ...
  endpoints: [
    ...
    { method: 'GET', path: '/api/practice/catalog?level=A1&limit=6' },
